...
object ExuComplexIssueInwardNodeImpl extends InwardNodeImp[Seq[RsParam], UpwardParam, (RsParam, UpwardParam, Parameters), IssueBundle]{
  override def edgeI(pd: Seq[RsParam], pu: UpwardParam, p: Parameters, sourceInfo: SourceInfo): (RsParam, UpwardParam, Parameters) = {

    (RsParam(3), pu, p)
  }
  override def bundleI(ei: (RsParam, UpwardParam, Parameters)): IssueBundle = new IssueBundle()
  override def render(ei: (RsParam, UpwardParam, Parameters)): RenderedEdge = RenderedEdge("#0000ff")
}
object ExuComplexIssueOutwardNodeImpl extends OutwardNodeImp[Seq[RsParam], TestParam, (RsParam, TestParam, Parameters), IssueBundle]{
  override def edgeO(pd: Seq[RsParam], pu: TestParam, p: Parameters, sourceInfo: SourceInfo): (RsParam, TestParam, Parameters) = {
    (RsParam(3), pu, p)
  }

  override def bundleO(eo: (RsParam, TestParam, Parameters)): IssueBundle = new IssueBundle()
}
class ExuComplexIssueNode(implicit valName: ValName) extends
  MixedNexusNode(inner = ExuComplexIssueInwardNodeImpl, outer = ExuComplexIssueOutwardNodeImpl)(
    ...
    uFn = {p:Seq[TestParam] => UpwardParam(p.head.width)}
  )
